...
describe("Controller", function () {
  async function deployControllerFixture() {
    ...
    const pcspToken = await ethers.deployContract("PostCovidStrokePrevention", [1000000000, 15000, 3000, 225, 30]);
  ...
  describe("Upload Data", function () {
    it("Should receive session id", async function () {
      ...
      await expect(controller.uploadData("doc1")).to.emit(controller, "UploadData").withArgs("doc1", 0);
    });
    ...
    it("Should fail if the doc is submited", async function () {
      ...
      await expect(controller.connect(addr1).uploadData(docId)).to.emit(controller, "UploadData").withArgs("doc1", 0);
      await controller.connect(addr1).confirm(docId, contentHash, proof, sessionId, riskScore);

      await expect(controller.connect(addr1).uploadData(docId)).to.be.revertedWith("Doc already been submitted");
    });
  });
  ...
  describe("Confirm data", function () {
    ...
    it("Should fail if the session and document is not correspondent", async function () {
      const { controller, addr1 } = await loadFixture(deployControllerFixture);
